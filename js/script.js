    ...
    window.openPlanetPage = function(name) {
      // Store the current URL before opening the planet page
      const currentUrl = window.location.href;
      
      // Create a simple HTML page with planet info
      const planetPage = `
        <!DOCTYPE html>
        <html>
        <head>
    ...
    window.addEventListener('click', (event) => {
      // Ignore clicks on search results (handled separately)
    ...
    function animate() {
      ...
      if (targetPlanet) {
        ...
        zoomProgress += 0.01;
        const defaultPosition = { x: 0, y: 40, z: 120 };
